...
doc = """
Investment Game 1 - Practice
"""


class Constants(BaseConstants):
    name_in_url = 'investment_game1_practice'
...
class CheckInterest(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        if player.still_interested == "no":
            ...
            player.participant.vars['last_round_played_investment1_practice'] = last_round
...
class Results(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        if 'results_by_round_investment1_practice' not in player.participant.vars:
            player.participant.vars['results_by_round_investment1_practice'] = []

        player.participant.vars['results_by_round_investment1_practice'].append({
            'round_number_investment1_practice': player.round_number,
            'payoff_investment1_practice': player.total_profit,
            'cost_investment1_practice': total_cost,
            'endowment_investment1_practice': player.payoff,
            'additional_investment1_practice': player.additional_endowment,
            'consumption_investment1_practice': player.consumption_cost
        })
...
class AllResults(Page):
    ...
    def vars_for_template(player: Player):
        ...
        results_by_round_investment1_practice = player.participant.vars.get('results_by_round_investment1_practice', [])

        # Tentukan ronde terakhir yang dimainkan
        end_game = participant.vars.get('end_game', False)
        if end_game:
            last_round_investment1_practice = participant.vars.get('last_round_played_investment1_practice', 1)
        else:
            last_round_investment1_practice = player.round_number

        # Ambil nilai payoff dari ronde terakhir yang dimainkan
        final_endowment_investment1_practice = player.in_round(last_round_investment1_practice).payoff

        # Hitung total payoff dan total cost
        total_payoff_investment1_practice = sum(item['payoff_investment1_practice']\
                                                for item in results_by_round_investment1_practice)
        total_cost_investment1_practice = sum(item['cost_investment1_practice']\
                                              for item in results_by_round_investment1_practice)

        return {
            'results_by_round_investment1_practice': results_by_round_investment1_practice,
            'total_payoff_investment1_practice': total_payoff_investment1_practice,
            'total_cost_investment1_practice': total_cost_investment1_practice,
            'last_round_played_investment1_practice': last_round_investment1_practice,
            'final_endowment_investment1_practice': final_endowment_investment1_practice
        }
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        if participant.vars.get('end_game', False):
            last_round = participant.vars.get('last_round_played_investment1_practice', 1)
