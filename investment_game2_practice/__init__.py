...
doc = """
Investment Game 2 - Practice
"""


class Constants(BaseConstants):
    name_in_url = 'investment_game2_practice'
...
class CheckInterest(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        if player.still_interested == "no":
            ...
            player.participant.vars['last_round_played_investment2_practice'] = last_round

            # Tetapkan endowment ke nilai payoff ronde sebelumnya
            previous_round = player.in_round(last_round)
            player.endowment = previous_round.payoff + participant.dynamic_additional_endowment_practice
            player.payoff = player.endowment
...
class Game(Page):
    ...
    def vars_for_template(player: Player):
        ...
        previous_round_endowment = player.in_round(
            player.round_number - 1).payoff + participant.dynamic_additional_endowment_practice \
            if player.round_number > 1 else participant.dynamic_endowment_practice + \
                                            participant.dynamic_additional_endowment_practice
        player.endowment = previous_round_endowment
        player.additional_endowment = participant.dynamic_additional_endowment_practice
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        consumption_fix = sum_profit - participant.dynamic_consumption_cost_practice
        if sum_profit < participant.dynamic_consumption_cost_practice:
            player.consumption_cost = sum_profit
            player.payoff = 0
        else:
            player.consumption_cost = participant.dynamic_consumption_cost_practice
...
class Results(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        if 'results_by_round_investment2_practice' not in player.participant.vars:
            player.participant.vars['results_by_round_investment2_practice'] = []

        player.participant.vars['results_by_round_investment2_practice'].append({
            'round_number_investment2_practice': player.round_number,
            'payoff_investment2_practice': player.total_profit,
            'cost_investment2_practice': total_cost,
            'endowment_investment2_practice': player.payoff,
            'additional_investment2_practice': player.additional_endowment,
            'consumption_investment2_practice': player.consumption_cost
        })
...
class AllResults(Page):
    ...
    def vars_for_template(player: Player):
        ...
        results_by_round_investment2_practice = player.participant.vars.get('results_by_round_investment2_practice', [])

        # Tentukan ronde terakhir yang dimainkan
        end_game = participant.vars.get('end_game', False)
        if end_game:
            last_round_investment2_practice = participant.vars.get('last_round_played_investment2_practice', 1)
        else:
            last_round_investment2_practice = player.round_number

        # Ambil nilai payoff dari ronde terakhir yang dimainkan
        final_endowment_investment2_practice = player.in_round(last_round_investment2_practice).payoff

        # Hitung total payoff dan total cost
        total_payoff_investment2_practice = sum(item['payoff_investment2_practice'] for item in results_by_round_investment2_practice)
        total_cost_investment2_practice = sum(item['cost_investment2_practice'] for item in results_by_round_investment2_practice)

        return {
            'results_by_round_investment2_practice': results_by_round_investment2_practice,
            'total_payoff_investment2_practice': total_payoff_investment2_practice,
            'total_cost_investment2_practice': total_cost_investment2_practice,
            'last_round_investment2_practice': last_round_investment2_practice,
            'final_endowment_investment2_practice': final_endowment_investment2_practice
        }
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        if participant.vars.get('end_game', False):
            last_round = participant.vars.get('last_round_played_investment2_practice', 1)
            participant.dynamic_endowment_practice = player.in_round(last_round).payoff
        else:
            participant.dynamic_endowment_practice = player.payoff
